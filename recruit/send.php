...
/* ===== スパム対策（外部サービス不要）=====
   いずれも「ボットとみなしたら、メールを一切送らずに完了画面へ無言でリダイレクト」。
   正規の応募者には影響しない。                                          */
// 1) ハニーポット：人間には見えない隠し項目。値が入っていればボット。
if (v('website') !== '') {
    header("Location: " . $thanks_url);
    exit;
}
// 2) 最短入力時間：JSが埋めた表示時刻より極端に速い送信はボットとみなす（3秒未満）。
//    JS無効でts未送信の場合(=0)はこの判定をスキップ（ハニーポットで防御）。
$ts = (int)v('ts');
if ($ts > 0) {
    $elapsed_ms = (int)(microtime(true) * 1000) - $ts;
    if ($elapsed_ms < 3000) {
        header("Location: " . $thanks_url);
        exit;
    }
...
/* 過大な入力（スパム／攻撃の典型）を拒否 */
if (mb_strlen($name) > 100 || mb_strlen($kana) > 100 || mb_strlen($address) > 200 ||
    mb_strlen($education) > 500 || mb_strlen($career) > 3000 || mb_strlen($motivation) > 3000) {
    show_error("入力された文字数が上限を超えています。");
}
